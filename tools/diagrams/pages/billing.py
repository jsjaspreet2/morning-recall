...
b.box(200,90,420,60,"Inference Gateway",["admit sync ≤5 ms · usage → local outbox in the run's txn","one event per run, at completion — never at submit"])
...
b.box(540,200,420,56,"Kafka usage.raw",["key = org_id · acks=all · 7-day retention","input · cached · output · reasoning tokens"])
...
b.arrow((540,326),(450,326)); b.ctext(495,282,"materialized balance",'dg-lbl')
...
a.box(190, 118, 200, 64, "Inference Gateway",
      ["usage event → outbox, same txn", "one per run, at completion"])
...
a.box(206, 252, 180, 64, "Admission Service",
      ["in-process leases", "reserves the worst case"])
...
a.queue(666, 252, 298, 56, "Kafka usage.raw",
        ["key org_id · acks=all", "input · cached · output · reasoning"])
a.box(666, 332, 298, 72, "Rate & post workers",
      ["dedupe on run_id", "prices all four token kinds",
       "at the price effective at occurred_at"])
a.cyl(666, 420, 298, 56, "ClickHouse", ["ORDER BY (org_id, occurred_at)"])
a.arrow((815, 308), (815, 332)); a.arrow((815, 404), (815, 420))
a.arrow((340, 182), (340, 200), (815, 200), (815, 252))
a.ctext(577, 192, "outbox pump", 'dg-lbl')
a.arrow((666, 360), (630, 360), (630, 300), (594, 300))
a.text(672, 318, "materialized balance", 'dg-lbl')
...
a.text(20, 738, "Prompt tokens are metered once, at completion, in the same event as the output — never at submission. At admission they only size the reservation.", 'dg-s')
