...
a.cyl(340, 228, 250, 64, "Inventory — DynamoDB", ["available to promise, per (sku, fc)", "on_hand - reserved >= qty"])
...
a.box(36, 652, 300, 64, "Fulfillment", ["plan · pick · pack · ship", "the pick is the point of no return"])
...
b = Board(660, "Place Order flow, as a sequence with its branches. The idempotency check comes first and short-circuits a double click. Then the quote is validated, with an expired quote branching to a 409 and a re-quote. Then inventory is reserved, with out-of-stock branching to a clean failure. Then payment is authorized, with a decline releasing the reservation. Then the order commits in one transaction. Below the commit, the outbox publishes to fulfillment, which picks and ships, and capture happens per shipment. A separate branch shows the cancel request racing the pick, winning or losing.")
...
b.box(30, 424, 300, 56, "outbox → Kafka → fulfillment", ["at-least-once, idempotent consumer"])
...
b.box(400, 512, 570, 56, "the race resolves, and either way is normal",
      ["won  →  void the auth, release the stock, order CANCELED",
...
s.box(350, 356, 300, 56, "Fulfillment picks", ["POINT OF NO RETURN, ~30 min"], cls='dg-warn', tcls='dg-warn-t', badge=8)
