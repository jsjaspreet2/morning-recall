...
a.box(206, 118, 150, 64, "API Gateway", ["HTTPS · auth · routing"])
a.box(376, 118, 158, 64, "Chat Service", ["POST /chats/{id}/messages", "quota check, then enqueue"])
...
a.box(806, 118, 158, 64, "GPU workers", ["continuous batching", "XADD tokens, then produce"])
a.arrow((160, 136), (206, 136)); a.ctext(183, 128, "POST", 'dg-lbl')
a.arrow((550, 150), (596, 150)); a.arrow((760, 150), (806, 150))
...
a.text(340, 272, "XADD per token", 'dg-lbl')
a.text(866, 272, "one message", 'dg-lbl')
a.text(340, 384, "XREAD from last id", 'dg-lbl')
a.text(866, 384, "consume", 'dg-lbl')
a.cyl(190, 290, 280, 64, "Redis Streams — token log", ["run:{runId}, one entry per token", "lossy, 10-minute TTL"])
a.queue(730, 290, 250, 64, "Kafka", ["key = chatId · one message"])
a.box(190, 400, 280, 64, "Streaming instances",
      ["GET /runs/{id}/stream — SSE", "holds the socket, owns no run state"])
a.box(730, 400, 250, 64, "Persister", ["consumes Kafka, batch-writes"])
a.arrow((330, 354), (330, 400)); a.arrow((855, 354), (855, 400))
a.cyl(190, 510, 280, 64, "Redis — scheduler + quota",
      ["sorted set per tier, score = enqueue + aging", "quota counters, sliding window, fails open"])
a.cyl(520, 510, 180, 64, "S3", ["cold chats"])
a.cyl(730, 510, 250, 64, "ScyllaDB", ["chats · messages · runs"])
a.arrow((855, 464), (855, 510))
a.arrow((670, 182), (670, 206), (490, 206), (490, 478), (330, 478), (330, 510))
a.text(500, 455, "dequeue", 'dg-lbl')
a.arrow((455, 182), (455, 232), (178, 232), (178, 542), (190, 542))
a.arrow((534, 150), (560, 150), (560, 486), (790, 486), (790, 510))
a.text(566, 478, "read history", 'dg-lbl')
a.arrow((190, 432), (170, 432), (170, 164), (160, 164)); a.ctext(183, 180, "SSE", 'dg-lbl')
