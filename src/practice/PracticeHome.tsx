  ...
  return (
    <div className="px-4 py-6 pb-28">
      <div className="flex items-baseline justify-between">
        <h1 className="text-2xl font-bold text-zinc-50">{meta.title}</h1>
        {streak > 0 && (
          <span className="text-sm text-amber-300 tabular-nums">🔥 {streak}-day streak</span>
        )}
      </div>
      ...
      <div className="mt-6 grid grid-cols-1 gap-3">
        <button
          ...
          className="group flex items-center justify-between rounded-2xl bg-zinc-100 text-zinc-900 px-5 py-4 text-left active:bg-white"
        >
          <span>
            <span className="block text-base font-semibold">Daily mix</span>
        ...
        <button
          ...
          className="group flex items-center justify-between rounded-2xl bg-zinc-900/50 ring-1 ring-zinc-800 px-5 py-4 text-left hover:ring-zinc-700"
        >
          <span>
            <span className="block text-base font-semibold text-zinc-100">Everything</span>
      ...
      {/* targeted areas */}
      <h2 className="mt-8 text-xs uppercase tracking-wide text-zinc-500">Focus on an area</h2>
      <p className="mt-1 text-sm text-zinc-500">Tap one or more, then start.</p>
            ...
            <button
              ...
              className={[
                ...
                on
                  ...
                  : 'bg-zinc-900/40 ring-zinc-800 hover:ring-zinc-700',
              ].join(' ')}
            >
              ...
              <span className="text-xs text-zinc-500 tabular-nums shrink-0">{t.count}</span>
            </button>
          )
        })}
      </div>
      ...
      {selected.length > 0 && (
        <div className="fixed inset-x-0 bottom-0 z-30 px-4 pb-[calc(1rem+env(safe-area-inset-bottom))] pt-3 bg-gradient-to-t from-[#0a0a0b] via-[#0a0a0b]/95 to-transparent">
          <div className="max-w-2xl mx-auto flex items-center gap-3">
            <button
              onClick={() => setSelected([])}
              className="text-sm text-zinc-400 px-3 py-3.5"
            >
              Clear
            </button>
            <button
              onClick={startSelected}
              className="flex-1 py-3.5 rounded-xl text-base font-semibold bg-emerald-500/90 text-emerald-950 active:bg-emerald-400"
            >
