...
| **GPU credit allocator** — org → project → user credits, jobs on a shared GPU pool | **Accrues continuously** per GPU-second while the job runs, at a price pinned when it was admitted | §8's reserve-then-settle becomes **reserve a hold for one heartbeat window, stream-settle each slice, refresh the hold** — the write rate drops to one ledger entry per job per heartbeat, and crash exposure is one window. Two things join the design: an **account hierarchy with transfers** (cross-shard transfers are a saga through an escrow account, never 2PC on a balance row) and a **scheduler contract** — admission returns a signed lease with a rate and an expiry, and the node agent drains the job when an extension is denied. Budget, quota, rate limit, and **fair-share** stay four distinct controls; a credit-rich tenant can still wait its turn. `openai-screen §06 E` works it in the screen's shape |
